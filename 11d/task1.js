...
const addStartingItems = (inputArgs) => {
  ...
  const items = rawItems.map((i) => parseInt(i.replace("/,", "/")));
  getLastMonkey().items = items;
};

const addOperation = (inputArgs) => {
  const raw1 = inputArgs[3]; // always old, so ignore
  const rawOperator = inputArgs[4]; // + / *
  const raw2 = inputArgs[5]; // old or number

  const operation = (input) => {
    const secondValue = raw2 === "old" ? input : parseInt(raw2);
    if (rawOperator === "+") {
...
const addTest = (inputArgs) => {
  const divisableBy = parseInt(inputArgs[3]);
  getLastMonkey().test = (input) => input % divisableBy === 0;
};
